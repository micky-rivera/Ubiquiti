...
const Form = () => {
    ...
    return (
        <>
            ...
            <form className="search-form" onSubmit={e => {e.preventDefault();}} data-testid='search-form'>
                <input className="search-form__input" type="text" placeholder="Search" value={userInput} onChange={e => {
                    setUserInput(e.target.value);
                    dispatch(setSearch(e.target.value));
                }}  />
            </form>
            <button className='search-form__clear-btn' onClick={clearForm} data-testid='search-form__clear-btn'>✕</button>
        </>
    );
}
